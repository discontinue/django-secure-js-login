...
print("\n\n=== 1. Create a new User ===\n")
print("# Client get's new, random **user salt** from server:", end=' ')
salt = "s_123"
print("'//%s//'" % salt)

print("# Password input on the client:", end=' ')
password = "client_password"
print("'//%s//'" % password)

print("# sha1(password + salt):", end=' ')
...
print("# Server split sha1 values:", end=' ')
sha1_a = sha1sum[:16]
sha1_b = sha1sum[16:]
print("**sha1_a**: '//%s//' **sha1_b**: '//%s//'" % (sha1_a, sha1_b))

print("# {{{encrypt(sha1_a, key=sha1_b)}}}:", end=' ')
...
print("# Client request login and get's a random **challenge** from server:", end=' ')
challenge = "c_123"
print("'//%s//'" % challenge)
...
print("# on the client: {{{sha1(password + salt)}}}:", end=' ')
sha1sum = sha1(password + salt)
print("'//%s//'" % sha1sum)

print("# on the client: split sha1 in:", end=' ')
sha1_a = sha1sum[:16]
sha1_b = sha1sum[16:]
print("**sha1_a**: '//%s//' **sha1_b**: '//%s//'" % (sha1_a, sha1_b))
...
print("# client generate **sha1_a** with e.g.: {{{sha1(sha_a, i, challenge, cnonce) x loop count}}}: \\\\", end=' ')
for i in range(LOOP_COUNT):
    sha1_a = sha1("%s%s%s%s" % (sha1_a, i, challenge, cnonce))
print("'//%s//'" % sha1_a)
...
print("# {{{decrypt(sha1checksum, key=sha1_b)}}}:", end=' ')
sha1checksum = decrypt(sha1checksum, key=sha1_b)
print("'//%s//'" % sha1checksum)

print("# server-site {{{sha1(sha1checksum, i, challenge, cnonce) x loop count}}}:", end=' ')
for i in range(LOOP_COUNT):
    sha1checksum = sha1("%s%s%s%s" % (sha1checksum, i, challenge, cnonce))
print("'//%s//'" % sha1checksum)

print("# compare **client site generated** SHA1 with the **server site generated**: \\\\", end=' ')
